...
//   const [searchStation, setSearchStation] = useState("");
//   const [newCoords, setNewCoords] = useState(coords);
//   const [userFavorites, setUserFavorites] = useState([]); // State to hold user's favorite stations
//   const [newData, setNewData] = useState(stationData);

//   const isFavorite = (stationId) => {
//     // Check if stationId exists in user's favorites
//     if (userFavorites && userFavorites.length > 0) {
//       return userFavorites.some((favorite) => favorite.stationId === stationId);
//     }
//     return false;
//   };
...
//     setUserFavorites([...userFavorites, { stationId }]);
//   };

//   useEffect(() => {
//     // Fetch user's favorite stations data
//     // Assuming this data is fetched from the backend
//     // Example API call: axios.get("/api/user/favorites")
//     const fetchUserFavorites = async () => {
//       try {
//         const response = await axios.get("/api/user/favorites");
//         setUserFavorites(response.data.favorites); // Assuming the response contains user's favorite stations
//       } catch (error) {
...
//                 <div className="space-x-4 text-gray-100">
//                   <button
//                     onClick={() => setNewCoords({ lat: item.lat, lng: item.log })}
//                     className="bg-blue-400 px-6 py-2"
//                   >
//                     See On Map
//                   </button>
//                   <Link to={"/station/" + item.id}>
//                     <button className="bg-red-400 px-6 py-2">Book A Slot</button>
//                   </Link>
//                   {isFavorite(item.id) ? (
//                     <button className="bg-green-400 px-6 py-2" disabled>
//                       Added
//                     </button>
//                   ) : (
//                     <button
//                       className="bg-orange-400 px-6 py-2"
...
//               {newData.map((item) => {
//                 return (
//                   <Link to={"/station/" + item.id} key={item.id} lat={item.lat} lng={item.log}>
//                     <div className="w-20 bg-gray-300 p-0.5 hover:scale-110">
//                       {item.image.length > 0 ? (
//                         <img
//                           className="w-full aspect-video bg-cover bg-center"
